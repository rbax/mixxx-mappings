...
//This mapping for the Pioneer DDJ-WeGO was made by Ryan Baxter
...
// this video shows how the lights are intended to be displayed
...
//   80-95 = from black to red leading greeny yellow spiral cw into 1/2 greeny yellow.
...
PioneerDDJWeGO.wheelTouch = function (channel, control, value, status, group) {
    if ((status & 0xF0) === 0x90) {    // If button down
		...
		//if (value === 0x90) {  // Some wheels send 0x90 on press and release, so you need to check the value
...
PioneerDDJWeGO.wheelTurn = function (channel, control, value, status, group) {
	...
	{		
			...
			PioneerDDJWeGO.wheelTouch(channel, control, value, status, group);
		
        engine.scratchTick(channel + 1, newValue); // Scratch!
		iscratch = true;
    } else {
		engine.scratchDisable( channel + 1 );
...
//DJ David
PioneerDDJWeGO.SelectNextTrack_or_prevTrack = function(channel, control, value, status, group) {
	if (value==0x01){
...
PioneerDDJWeGO.SelectNextTrack_or_prevPlaylist = function(channel, control, value, status, group) {
	if (value==0x01){
